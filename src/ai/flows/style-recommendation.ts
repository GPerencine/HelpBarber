...
const StyleRecommendationInputSchema = z.object({
  faceShape: z.string().describe('O formato do rosto do usuário (ex: Redondo, Oval, Quadrado).'),
  hairType: z.string().describe('O tipo de cabelo do usuário (ex: Liso, Ondulado, Cacheado).'),
  occasion: z
    .string()
...
const StyleRecommendationOutputSchema = z.object({
  hairstyleRecommendation: z
    .string()
    .describe(
      'Um penteado ou corte de cabelo masculino sugerido com base nos parâmetros de entrada.',
    ),
  styleDescription: z
    .string()
    .describe('Uma descrição do penteado ou corte de cabelo masculino sugerido.'),
});
