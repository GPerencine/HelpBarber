          ...
          <Tabs defaultValue="barbers" className="w-full">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="barbers">
                <Scissors className="mr-2" /> Barbeiros
              </TabsTrigger>
              <TabsTrigger value="users">
                <User className="mr-2" /> Clientes
              </TabsTrigger>
              <TabsTrigger value="reviews">
                <Star className="mr-2" /> Avaliações
              </TabsTrigger>
            </TabsList>
            <TabsContent value="barbers">
              <Card className="shadow-xl border-primary/10 bg-card/80 backdrop-blur-md">
                <CardHeader>
                  ...
                  <CardDescription>
                    Lista de todos os barbeiros cadastrados e status de localização.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {barbers &&
                    barbers.map((barber) => (
                      <div
                        key={barber.id}
                        className="flex items-center gap-4 rounded-xl bg-background/50 border border-border/40 p-4 hover:bg-muted/40 hover:-translate-y-1 hover:shadow-md transition-all duration-300"
                      >
                        ...
                        <div className="flex-1">
                          <p className="font-semibold flex items-center gap-2">
                            {barber.name}
                            {barber.coordinates ? (
                              <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full uppercase font-bold tracking-wider">
                                No Mapa
                              </span>
                            ) : (
                              <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full uppercase font-bold tracking-wider">
                                Sem Localização
                              </span>
                            )}
                          </p>
                      ...
                      >
                        ...
                        <AlertDialogTrigger asChild>
                          <Button
                            ...
                            size="icon"
                            onClick={() =>
                              setItemToDelete({ id: user.id, name: user.name, type: 'user' })
                            }
                          >
                            <Trash2 className="h-4 w-4" />
                          </Button>
              ...
              <Card className="shadow-xl border-primary/10 bg-card/80 backdrop-blur-md">
                ...
                <CardContent className="space-y-4">
                  {enrichedReviews.map((review) => (
                    <div
                      key={review.id}
                      className="flex items-start gap-4 rounded-xl bg-background/50 border border-border/40 p-4 hover:bg-muted/40 hover:-translate-y-1 hover:shadow-md transition-all duration-300"
                    >
