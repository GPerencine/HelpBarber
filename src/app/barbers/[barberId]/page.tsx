...
const BarberMap = dynamic(() => import('@/components/barbers/barber-map'), {
  loading: () => (
    <div className="h-[300px] md:h-[400px] w-full bg-muted rounded-lg flex items-center justify-center">
...
export default function BarberProfilePage() {
  ...
  const barberRef = useMemo(
    () => (firestore && barberId ? doc(firestore, 'barbers', barberId) : null),
    [firestore, barberId],
  );

  const { data: barber, isLoading: isBarberLoading } = useDoc<Barber>(barberRef);
  const { data: appointments, isLoading: areAppointmentsLoading } =
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 text-muted-foreground">
                  <div className="flex items-center">
                    <MapPin className="mr-2 h-4 w-4" /> {barber.location}
                  </div>
                  <div className="flex items-center">
                    <Trophy className="mr-2 h-4 w-4" /> {barber.experience} anos de experiência
                  </div>
                  <div className="flex items-center">
                    <Star className="mr-2 h-4 w-4 text-yellow-500 fill-yellow-500" />{' '}
                    {averageRating.toFixed(1)} média
                  </div>
                </div>
              </div>
            </CardHeader>
