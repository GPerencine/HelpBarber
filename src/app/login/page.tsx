        ...
        <CardContent className="pb-10">
          <div className="flex items-center justify-center space-x-2 mb-8 bg-muted/30 p-1.5 rounded-2xl border border-border/20">
            <Label
              htmlFor="user-type"
              className={cn(
                'cursor-pointer flex-1 flex items-center justify-center px-4 py-2.5 rounded-xl transition-all text-xs font-bold',
                !isBarberLogin
            ...
            <Switch
              id="user-type"
              checked={isBarberLogin}
              onCheckedChange={(val) => {
                setIsBarberLogin(val);
                setError(null);
              }}
            />
            <Label
          ...
          <Form {...loginForm}>
            <form onSubmit={loginForm.handleSubmit(onLoginSubmit)} className="space-y-5">
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        className="h-12 bg-muted/20 border-muted focus:ring-primary/20"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
              ...
              />
              {error && (
                <p className="text-xs font-bold text-destructive text-center bg-destructive/10 p-3 rounded-xl border border-destructive/20 animate-in slide-in-from-top-2">
                  {error}
                </p>
              )}
              <div className="text-right">
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button
                      variant="link"
                      className="p-0 h-auto text-xs font-bold opacity-70 hover:opacity-100"
                    >
                      Esqueci minha senha
                    </Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent className="w-[95%] max-w-md rounded-3xl glass-card">
                    ...
                    <AlertDialogFooter className="gap-2">
                      ...
                      <AlertDialogAction
                        onClick={() => onPasswordReset(loginForm.getValues('email'))}
              ...
              <Button
                type="submit"
                className="w-full h-14 text-lg font-black premium-gradient shadow-xl shadow-primary/30 group"
                disabled={isLoading}
              >
                {isLoading ? (
                  <>
