  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      setDocumentNonBlocking(
        doc(firestore, 'barbers', barber.id, 'appointments', appointmentId),
        app,
        { merge: true },
      );
      setDocumentNonBlocking(
        doc(firestore, 'users', user.uid, 'appointments', appointmentId),
        app,
        { merge: true },
      );
      ...
      toast({
        ...
        action: (
          <ToastAction altText="Ver Agendamentos" onClick={() => router.push('/meus-agendamentos')}>
            Ver
          </ToastAction>
        ),
      });
    ...
    <Card className="shadow-lg border-none">
      <CardHeader className="bg-primary text-primary-foreground rounded-t-lg">
        <CardTitle className="flex items-center gap-2">
          <CalendarIcon className="h-5 w-5" /> Agendar Horário
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6 pt-6">
        ...
        <div
          className={cn(!selectedService && 'opacity-50 pointer-events-none transition-opacity')}
        >
          <h4 className="mb-3 text-sm font-bold uppercase text-muted-foreground">
            2. Selecione a Data
          </h4>
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className="w-full h-12 justify-start font-medium border-2 hover:border-primary/50"
              >
        ...
        <AlertDialog open={showLoginPrompt} onOpenChange={setShowLoginPrompt}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogDescription>
                Para realizar um agendamento, você precisa estar conectado à sua conta.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription className="text-base">
                  ...
                  às <span className="font-bold text-foreground">{selectedTime}</span>.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Ajustar</AlertDialogCancel>
                <AlertDialogAction onClick={handleConfirmBooking} className="font-bold">
                  Confirmar
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
