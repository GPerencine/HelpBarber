...
const barberNavLinks = [{ href: '/barber-dashboard', label: 'Dashboard', icon: LayoutDashboard }];

const adminNavLinks = [{ href: '/admin', label: 'Admin', icon: Shield }];
...
export default function Header() {
    ...
    <div className="flex items-center gap-2">
      {isUserLoading ? (
        ...
        <div className="flex items-center gap-3">
          <Link
            href={isBarber ? '/barber-dashboard' : '/perfil'}
          ...
          <Button
            ...
            size="sm"
            className="hidden sm:flex hover:text-destructive hover:bg-destructive/5"
          >
            ...
            {isUserLoading ? (
              ...
              <>
                ...
                <Button
                  variant="ghost"
                  onClick={handleSignOut}
                  className="justify-start h-12 text-destructive hover:bg-destructive/5 hover:text-destructive"
                >
  ...
  const mainLink = (
    <Link
      href={user ? (isBarber ? '/barber-dashboard' : '/barbers') : '/'}
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/80 backdrop-blur-xl supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 items-center px-4">
        <div className="mr-8 hidden md:flex">{mainLink}</div>

        {mobileNav}

        <div className="flex flex-1 items-center justify-between space-x-2 md:justify-end">
          <div className="md:hidden">{mainLink}</div>
          <div className="flex-1 md:flex-none">{desktopNav}</div>
